...
export default function Theme10() {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        if (user?.id) {
          const res = await axios.get(`http://172.20.10.3:5000/api/theme2/${user.id}`);
  ...
  const items = [
    {
      id: 1,
      image: require('@/assets/images/objA1.png'),
      locked: avancer < 7,
      route: '/ex3_t2_1',
    },
    {
      id: 2,
      image: require('@/assets/images/objA3.png'),
      locked: avancer < 8,
      route: '/ex3_t2_2',
    },
    {
      id: 3,
      image: require('@/assets/images/objA2.png'),
      locked: avancer < 9,
      route: '/ex3_t2_3',
    },
  ];
  ...
  return (
    <>
      ...
      <View style={styles.background}>
        <Image source={require('@/assets/images/Fonden-degrade1.png')} style={styles.img} />
      </View>

      <ScrollView style={styles.contain}>
        <View style={styles.top}>
          <Text style={styles.titre}>🎯 Niveau 3</Text>
          <Text style={styles.text}>Débloquez les niveaux suivants en complétant les exercices précédents.</Text>
        </View>

        <View style={styles.containt}>
          <View style={styles.river}>
            <Image source={require('@/assets/images/Forme1.png')} style={styles.img} />
          </View>
