...
export default function Theme0() {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        if (user?.id) {
          const res = await axios.get(`http://172.20.10.3:5000/api/theme0/${user.id}`);
  ...
  const items = Array.from({ length: 3 }, (_, i) => ({
    id: i + 1,
    image: require('@/assets/images/obC1.png'),
    locked: i + 1 > avancer,
    route: `/ex1_t0_${i + 1}`
  }));
  ...
  return (
    <>
      ...
      <View style={styles.background}>
        <Image source={require('@/assets/images/Fonden-degrade2.png')} style={styles.img} />
      </View>

      <ScrollView style={styles.contain}>
        <View style={styles.top}>
          <Text style={styles.titre}>🎯 Niveau 1</Text>
          <Text style={styles.text}>Débloquez les niveaux suivants en complétant les exercices précédents.</Text>
        </View>

        <View style={styles.containt}>
          <View style={styles.river}>
            <Image source={require('@/assets/images/Forme2.png')} style={styles.img} />
          </View>
