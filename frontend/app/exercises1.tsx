...
import ThemeComponent from "@/components/exercises1";
...
export default function Ex2T0_2Page() {
  ...
  useEffect(() => {
    ...
    // Animation de pulsation pour le badge "Ghomala"
    Animated.loop(
      Animated.sequence([
  ...
  const renderTab = (label, iconName, route, isLogo = false, badge = null) => {
    const isActive = route === "/exercises1";
    ...
    return (
      ...
      >
        ...
        >
          {isLogo ? (
            <IconSymbol size={26} name="house.fill" color={Colors.gris3} />
          ) : (
            <Ionicons
              name={iconName}
              size={26}
    ...
    <>
      ...
      >
        <View style={styles.tabs}>
          {renderTab("Leçons", "book-outline", "/sage1")}
          {renderTab("Exercices", "trophy-outline", "/exercises1", false, "Ghomala")}
          {renderTab("Accueil", "", "/home3", true)}
          {renderTab("Communauté", "people-outline", "/top1")}
          {renderTab("Profil", "person-outline", "/profil1")}
        </View>
      </Animated.View>
    </>
  );
