...
const colors = [
  { label: 'j ai : Gaə̂ gə', file: require('@/assets/sounds/ghomala/verbe2/1.wav') },
  { label: 'tu as : ô gə', file: require('@/assets/sounds/ghomala/verbe2/2.wav') },
  { label: 'il ou elle as : ě gə', file: require('@/assets/sounds/ghomala/verbe2/3.wav') },
  { label: 'Nous avons : pyə̂ gə', file: require('@/assets/sounds/ghomala/verbe2/4.wav') },
  { label: 'vous avez : pô gə', file: require('@/assets/sounds/ghomala/verbe2/5.wav') },
  { label: 'ils ont : wâp gə', file: require('@/assets/sounds/ghomala/verbe2/6.wav') },
 
];

export default function ghomalaColorLesson() {
  ...
  const handlePlay = async (index: number) => {
    try {
      setIsPlaying(index);
      animateMascot();
      const { sound } = await Audio.Sound.createAsync(colors[index].file);
      await sound.playAsync();
  ...
  const isLessonCompleted = played.length === colors.length;
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.title}> Leçon 3 : Le verbe avoir en ghomala</Text>
        <Text style={styles.instruction}>🎧 Appuie sur chaque couleur pour entendre sa prononciation</Text>
  ...
  <Animated.View style={[styles.confettiContainer, { opacity: successAnim }]}>
    ...
    <Text style={styles.confettiText}>🎉 Bravo !</Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push('/ghomala/lesson4')}
        >
